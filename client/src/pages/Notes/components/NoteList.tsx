   ...
   return (
      <div className="flex flex-col gap-4 w-full">
         ...
         <div className="flex justify-between items-center px-1">
            <h1 className="font-sans text-[25px] font-black text-gray-900 dark:text-white tracking-tight">Ghi chú</h1>
            <div className="flex items-center gap-3">
               <motion.button
                  onClick={handleCreateNote}
                  whileTap={{ scale: 0.95 }}
                  className="flex items-center gap-1.5 px-5 py-2 bg-[#0A84FF]/10 hover:bg-[#0A84FF]/16 text-[#0A84FF] rounded-xl font-sans text-xs font-bold transition-all cursor-pointer border-none"
               >
               ...
               <motion.button
                  ...
                  whileTap={{ scale: 0.95 }}
                  className="font-sans text-xs font-bold text-[#0A84FF] hover:opacity-80 cursor-pointer bg-transparent border-none"
               >
                  Trang chủ
               </motion.button>
            </div>
         ...
         {isLoading ? (
            <div className="flex flex-col items-center justify-center py-20 gap-3">
               <Loader2 size={24} className="text-[#0A84FF] animate-spin" />
            ...
            <div className="flex flex-col items-center justify-center py-20 gap-3 text-center px-4">
               ...
               <h4 className="font-sans font-black text-gray-800 dark:text-zinc-300 text-md">
            ...
            <div className="bg-white dark:bg-[#1C1C1E] border border-black/5 dark:border-white/6 rounded-2xl overflow-hidden divide-y divide-black/4 dark:divide-white/4">
               {filteredNotes.map((note) => (
                  <div
                     key={note._id}
                     onClick={() => setActiveNote(note)}
                     className="p-4 hover:bg-[#0A84FF]/04 dark:hover:bg-[#0A84FF]/04 active:bg-[#0A84FF]/08 dark:active:bg-[#0A84FF]/08 cursor-pointer flex flex-col gap-1 transition-all"
                  >
