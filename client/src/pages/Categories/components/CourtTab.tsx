...
import { Pencil, Trash2, MapPin, Search, Calendar, Landmark, Info } from "lucide-react";
      ...
      <div className="space-y-6">
         ...
         <section className="animate-fade-in">
            ...
            <div className="grid grid-cols-2 gap-4">
               <div className="glass-card rounded-3xl p-4 flex flex-col justify-between min-h-27.5 transition-all hover:shadow-[0_12px_24px_rgba(216,180,254,0.2)]">
                  <span className="font-sans text-[11px] font-bold text-gray-500 uppercase tracking-wider flex items-center gap-1.5">
                     <Landmark size={14} className="text-[#6f5092]" /> Tổng số sân
                  </span>
                  <div className="mt-2 flex flex-col">
                     <span className="font-sans text-3xl font-extrabold text-[#6f5092] tracking-tight">{totalCourts}</span>
               ...
               <div className="glass-card rounded-3xl p-4 flex flex-col justify-between min-h-27.5 transition-all hover:shadow-[0_12px_24px_rgba(216,180,254,0.2)]">
                  <span className="font-sans text-[11px] font-bold text-gray-500 uppercase tracking-wider flex items-center gap-1.5">
                     <Calendar size={14} className="text-[#7b41b4]" /> Giá trung bình
                  </span>
                  <div className="mt-2 flex flex-col">
         ...
         <section className="space-y-3">
            <div className="flex justify-between items-center px-1">
               <h3 className="font-sans text-[15px] font-extrabold text-gray-500 uppercase tracking-wider">Danh sách ({filteredCourts.length})</h3>
            ...
            {filteredCourts.length === 0 ? (
               <div className="py-10 bg-white/40 backdrop-blur-md rounded-3xl border border-gray-100/50 flex flex-col items-center justify-center">
                  <Empty description="Không tìm thấy sân nào" image={Empty.PRESENTED_IMAGE_SIMPLE} />
               </div>
            ) : (
               <motion.div variants={containerVariants} initial="hidden" animate="show" className="space-y-3">
                  {filteredCourts.map((court) => (
                     <motion.div
                        ...
                        className="glass-card rounded-3xl p-4 flex flex-col gap-3 group hover:bg-white/95 transition-all select-none border border-white/50"
                     >
                        <div className="flex items-center gap-4">
                           {/* Court Avatar Icon */}
                           <div className="w-12 h-12 rounded-2xl bg-linear-to-br from-[#efdbff] to-[#ddb8ff] flex items-center justify-center text-[#7b41b4] shrink-0 shadow-sm">
                           ...
                           <div className="flex-1 min-w-0">
                              <div className="flex justify-between items-start">
                                 <h4 className="font-sans text-[15px] font-extrabold text-gray-800 truncate leading-snug pr-2">{court.name}</h4>
                        ...
                        {court.description && (
                           <div className="bg-gray-50/50 rounded-xl p-2.5 flex items-start gap-1.5 border border-gray-100/50">
                              <Info size={13} className="text-gray-400 shrink-0 mt-0.5" />
                              <p className="font-sans text-[11px] font-semibold text-gray-500 leading-normal">{court.description}</p>
                           </div>
                        )}
                        ...
                        {court.timeSlots && court.timeSlots.length > 0 && (
                           <div className="mt-1 space-y-2">
                              <span className="font-sans text-[10px] font-extrabold text-gray-400 uppercase tracking-wider block">
                                 Khung giờ & bảng giá:
                              </span>
                              <div className="flex gap-2 flex-wrap">
                                 {court.timeSlots.map((slot, index) => (
                                    <span
                                       key={index}
                                       className="bg-purple-50/70 text-[#7b41b4] px-2.5 py-1 rounded-lg font-sans text-[11px] font-extrabold border border-purple-100/40 shadow-xs flex items-center gap-1 hover:bg-[#efdbff] transition-all"
                                    >
                                       🕒 {slot.startHour} - {slot.endHour} :{" "}
                                       <span className="font-extrabold text-[#6f5092]">{slot.pricePerHour.toLocaleString("vi-VN")}đ</span>
